    ...
    void* data)
{
    ...
    if (self->interface_aidl == RADIO_AIDL_INTERFACE_NONE) {
        ...
        gbinder_writer_append_int32(&writer, cls);      /* serviceClass */
        gbinder_writer_append_string16(&writer, binder_sim_card_app_aid(self->card));
                                                        /* appId */
    }
...
{
    BinderCallBarring* self = ofono_call_barring_get_data(b);
    const RADIO_AIDL_INTERFACE iface_aidl = radio_client_aidl_interface(self->network_client);
