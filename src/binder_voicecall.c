    ...
    void* data)
{
    ...
    if (binder_voicecall_have_ext_call(self)) {
        if (!binder_voicecall_ext_swap(self, cbd, flags)) {
            DBG_(self, "%s (fallback)", radio_req_name(self->instance, fallback));
            binder_voicecall_request_submit(self, fallback, cbd);
        }
    } else {
        /* Default action */
        DBG_(self, "%s", radio_req_name(self->instance, fallback));
        binder_voicecall_request_submit(self, fallback, cbd);
    }
    binder_voicecall_cbd_unref(cbd);
}
        ...
        for (l = self->calls; l; l = l->next) {
            ...
            if (filter(call)) {
                ...
                if (call->ext) {
                    ...
                    DBG_(self, "%s %u", radio_req_name(self->instance, fallback), id);
                }
