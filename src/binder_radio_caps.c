    ...
    void* user_data)
{
        ...
        if (resp == code) {
            if (error == RADIO_ERROR_NONE) {
                if (check->interface_aidl == RADIO_AIDL_INTERFACE_NONE) {
                    ...
                    result->logicalModemUuid.data.str = gbinder_reader_read_string16(&reader);
                    result->logicalModemUuid.len = strlen(result->logicalModemUuid.data.str);
    ...
    if (check->interface_aidl == RADIO_MODEM_INTERFACE && result) {
        g_free(result->logicalModemUuid.data.str);
        g_free(result);
    }
}
